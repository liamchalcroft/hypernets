...
def unpack_dataset(folder, threads=default_num_threads, key="data"):
    ...
    npz_files = subfiles(os.path.join(folder,'meta'), True, None, ".npz", True)
    p.map(convert_to_npy, zip(npz_files, [key] * len(npz_files)))
    p.close()
    p.join()
...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    for c in case_identifiers:
        ...
        dataset[c]['meta_file'] = join(folder, "meta/%s.npz" % c)
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        self.data_shape, self.meta_shape, self.seg_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            ...
            meta_data = np.load(self._data[k]['meta_file'][:-4] + ".npy", self.memmap_mode)
        else:
            case_all_data = np.load(self._data[k]['data_file'])['data']
            meta_data = np.load(self._data[k]['meta_file'])['data']
        num_color_channels = case_all_data.shape[0] - 1
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg, *self.patch_size)
        meta_shape = (self.batch_size, meta_data.shape[0])
        return data_shape, meta_shape, seg_shape

    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        meta = np.zeros(self.meta_shape, dtype=np.float32)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            if not isfile(self._data[i]['data_file'][:-4] + ".npy"):
                ...
                meta_data = np.load(self._data[i]['meta_file'][:-4] + ".npz")['data']
            else:
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode)
                meta_data = np.load(self._data[i]['meta_file'][:-4] + ".npy", self.memmap_mode)
        ...
        return {'data': data, 'meta': meta, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        keys = selected_keys
        return {'data': data, 'meta': meta, 'seg': seg, 'properties': case_properties, "keys": keys}
