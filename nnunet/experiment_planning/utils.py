...
from hypunet.experiment_planning.common_utils import split_4d_nifti
from hypunet.paths import hypunet_raw_data, hypunet_cropped_data, preprocessing_output_dir
from hypunet.preprocessing.cropping import ImageCropper
...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    assert isdir(join(input_folder, "imagesTr")) and isdir(join(input_folder, "labelsTr")) and \
           isfile(join(input_folder, "dataset.json")) and isdir(join(input_folder, "metaTr")), \
    ...
    output_folder = join(hypunet_raw_data, "Task%03.0d_" % overwrite_task_output_id + task_name)
    ...
    shutil.copytree(join(input_folder, "metaTr"), join(output_folder, "metaTr"))
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for tr in training_files:
        ...
        cur_pat.append(join(base_folder_splitted, "metaTr", tr['meta'].split("/")[-1]))
...
def crop(task_string, override=False, num_threads=default_num_threads):
    cropped_out_dir = join(hypunet_cropped_data, task_string)
    ...
    splitted_4d_output_dir_task = join(hypunet_raw_data, task_string)
    lists, _ = create_lists_from_splitted_dataset(splitted_4d_output_dir_task)

    imgcrop = ImageCropper(num_threads, cropped_out_dir)
    imgcrop.run_cropping(lists, overwrite_existing=override)
    shutil.copy(join(hypunet_raw_data, task_string, "dataset.json"), cropped_out_dir)
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    from hypunet.experiment_planning.experiment_planner_baseline_2DUNet import ExperimentPlanner2D
    ...
    shutil.copy(join(hypunet_raw_data, task_string, "dataset.json"), preprocessing_output_dir_this_task_train)
