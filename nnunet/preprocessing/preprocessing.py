...
from hypunet.configuration import default_num_threads, RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD
from hypunet.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper
...
class GenericPreprocessor(object):
    ...
    def load_cropped(cropped_output_dir, case_identifier):
        ...
        meta_data = np.load(os.path.join(cropped_output_dir, "meta/%s.npz" % case_identifier))['data']
        data = all_data[:-1].astype(np.float32)
        seg = all_data[-1:]
        with open(os.path.join(cropped_output_dir, "%s.pkl" % case_identifier), 'rb') as f:
            properties = pickle.load(f)
        return data, meta_data, seg, properties
    ...
    def preprocess_test_case(self, data_files, target_spacing, seg_file=None, force_separate_z=None, met_file=None):
        data, seg, properties = ImageCropper.crop_from_list_of_files(data_files, met_file, seg_file)
    ...
    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir, force_separate_z,
                      all_classes):
        data, meta_data, seg, properties = self.load_cropped(cropped_output_dir, case_identifier)
        ...
        np.savez_compressed(os.path.join(output_folder_stage, "meta/%s.npz" % case_identifier),
                            data=meta_data.astype(np.float32))
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        for i in range(num_stages):
            ...
            maybe_mkdir_p(os.path.join(output_folder_stage,'meta'))
