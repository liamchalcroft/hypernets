...
class ImageCropper(object):
    ...
    def load_crop_save(self, case, case_meta, case_identifier, overwrite_existing=False):
        try:
            ...
            if overwrite_existing \
                ...
                meta_data = np.load(case_meta)
                np.savez_compressed(os.path.join(self.output_folder, "%s.npz" % case_identifier), data=all_data)
                np.savez_compressed(os.path.join(self.output_folder, "meta/%s.npz" % case_identifier), data=meta_data)
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        maybe_mkdir_p(os.path.join(self.output_folder, "meta"))
        ...
        for j, case in enumerate(list_of_files):
            ...
            list_of_args.append((case[:-1], case[-1], case_identifier, overwrite_existing))
