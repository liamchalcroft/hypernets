...
from hypunet.configuration import (
    default_num_threads,
...
class GenericPreprocessor(object):
    ...
    def load_cropped(cropped_output_dir, case_identifier):
        ...
        meta_data = np.load(
            os.path.join(cropped_output_dir, "meta/%s.npz" % case_identifier)
        )["data"]
    ...
    def _run_internal(
        ...
        data, meta_data, seg, properties = self.load_cropped(
        ...
        np.savez_compressed(
            os.path.join(output_folder_stage, "meta/%s.npz" % case_identifier),
