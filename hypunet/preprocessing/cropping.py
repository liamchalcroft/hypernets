...
class ImageCropper(object):
    ...
    def load_crop_save(
        self, case, case_meta, case_identifier, overwrite_existing=False
    ):
        try:
            ...
            if overwrite_existing or (
                ...
                np.savez_compressed(
                    os.path.join(self.output_folder, "meta/%s.npz" % case_identifier),
                    data=meta_data,
                )
                with open(
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        for j, case in enumerate(list_of_files):
            ...
            list_of_args.append(
                (case[:-1], case[-1], case_identifier, overwrite_existing)
            )
