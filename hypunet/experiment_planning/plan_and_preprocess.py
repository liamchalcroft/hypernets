...
def main():
    ...
    if planner_name3d is not None:
        planner_3d = recursive_find_python_class(
            [search_in], planner_name3d, current_module="hypunet.experiment_planning"
        )
    ...
    if planner_name2d is not None:
        planner_2d = recursive_find_python_class(
            [search_in], planner_name2d, current_module="hypunet.experiment_planning"
        )
    ...
    for t in tasks:
        ...
        shutil.copy(
            join(hypunet_raw_data, t, "dataset.json"),
            preprocessing_output_dir_this_task,
        )
