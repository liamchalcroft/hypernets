...
def split_4d(
    ...
    assert (
        ...
        and isdir(join(input_folder, "metaTr"))
    ), (
    ...
    output_folder = join(
        hypunet_raw_data, "Task%03.0d_" % overwrite_task_output_id + task_name
    )
...
def plan_and_preprocess(
    ...
    from hypunet.experiment_planning.experiment_planner_baseline_2DUNet import (
        ExperimentPlanner2D,
    )
    from hypunet.experiment_planning.experiment_planner_baseline_3DUNet import (
        ExperimentPlanner,
    )
    ...
    shutil.copy(
        join(hypunet_raw_data, task_string, "dataset.json"),
        preprocessing_output_dir_this_task_train,
    )
