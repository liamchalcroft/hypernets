...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]["data_file"][:-4] + ".npy"):
            ...
            meta_data = np.load(
                self._data[k]["meta_file"][:-4] + ".npy", self.memmap_mode
            )
        else:
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not isfile(self._data[i]["data_file"][:-4] + ".npy"):
                ...
                meta_data = np.load(
                    self._data[i]["meta_file"][:-4] + ".npy", self.memmap_mode
                )
            ...
            if not force_fg:
                ...
                if "class_locations" not in properties.keys():
                    raise RuntimeError(
                        "Please rerun the preprocessing with the newest version of hypunet!"
                    )
